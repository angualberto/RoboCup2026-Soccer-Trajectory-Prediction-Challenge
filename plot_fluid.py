...
for fname in files:
    ...
    for idx, agent in enumerate(agents):
        ...
        ax.plot(sub_idx, sx, 'r.-', label='pred x', linewidth=1.5, markersize=3)
        ax.plot(sub_idx, sy, 'r.-', alpha=0.5, linewidth=1.5, markersize=3, label='pred y')
        ax.plot(sub_idx, gx, 'g.-', label='gt x', linewidth=1.5, markersize=3, alpha=0.7)
        ax.plot(sub_idx, gy, 'g.-', alpha=0.5, linewidth=1.5, markersize=3, label='gt y')
        ...
        ax.set_xlabel('cycle')
        ax.set_ylabel('pos (m)')
    plt.suptitle(f'{fname} | Avg endpoint: {np.mean(all_errs):.1f}m | Fluid Ball (g=0.4, s=0.0)')
    plt.tight_layout()
    out = f'results/test/{fname.replace(".tracking.csv","")}_fluid_plot.png'
